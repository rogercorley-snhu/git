...
'           Date: 04-Jan-11
'
'                  This software is protected under copyright law.  It is forbidden to copy
'                  use or distrubute this software without the express, written, permission
'                  of Common CENTS Solutions.  No warranties are expressed or implied.  Use
'                  of this software is at the risk of the user and Common CENTS Solutions is 
'                  not libabe for any loss or damage resulting from its use.
'
'                  License to use this software is granted to the user having a valid, paid,
'                  software maintenance agreement in effect for a qualifying GEM product.
'
'                  (c) Copyright 2011, Common CENTS Solutions, Inc
'                  All rights reserved
'
'
'    Description:  This script runs verification checks on each field of an import file to verify the fields meet specific criteria. 
'				   Depending on the settings in the "Settings" area below, this script will create a "Good" import file containing all rows
'				   whose fields passed the verifications and also create a "Bad" import file containing only those rows for whom at least
'				   one field did NOT pass. If specified, the script will add a new field to the end of all rows in the "Bad" file explaining
'				   which field failed and the reason. 
'
'				   The "Config Array" variables should contain relevant values for each field (column) in the import file. Each array element 
'				   will correspond to the same ordinal field in each line (i.e. element 1 = field1, etc.). In other words each array variable 
'				   should have the same number of elements as the expected number of fields, even if you aren't checking each field.
'				   Failure to do so will result in an early script termination.
'
'				   All Errors, including one that cause the script to halt execution, will be logged in the specified log file unless
'				   a log file name isn't specified.
'
'				   
' Developer Notes: A developer can add new verifications to this script by adding functions to the verification functions section And
'				   calls to those functions within the "RunLineVerifications" procedure and also a Constant or an array varaible to define 
'				   parameters for each field in each row. Inspect existing functions, calls and constants to see how it is done.
'
...
'		---------------------------------------------------------------------------------------------------------------------------------------------------
'		* Minimum number of lines		none						There must be at least this number of lines in the file. use the MIN_NUMBER_OF_LINES constant to set this minimum
...
'						This means that the expected array should be an undefined, null, or non-existant variable. This also means that any custom Functions
'						need to implement this methodology, or the behavior will not work as intended. In fact, outside of this change to each function, this
'						version is identical to version 1.00.
'
'    Version 1.20   				    19-Mar-15       Richard Beverly
'			Feature: 	A minimum number of lines must exist in the file, otherwise the Good file will be archived and a new "good file" will be created as an empty file.
'						Use the MIN_NUMBER_OF_LINES constant to set this minimum. This will always be evaluated, so set a realistic value. That is, if you don't care to check
'						set to a very low number. If you KNOW there will be at least (for instance) 1,500 employees in the file everytime, and it would be an incorrect
'						file if it were less, then this number should be 1500 (do not use the comma thousands seperator)
'
'=======================================================================================
...
Const FILE_DIRECTORY 	= "" '<--- DO NOT include a '\' at end. empty string means current dir
Const INPUT_FILE_NAME 	= "testImport.txt"
Const BAD_FILE_NAME 	= "BadRecords.txt"
Const GOOD_FILE_NAME 	= "GoodRecords.txt"
Const LOG_FILE_NAME 	= "VerifyImportError.log"
Const FIELD_DELIMITER 	= ","
Const INCLUDE_EXPLAINATION_FIELD = True

Const EXPECTED_NUMBER_OF_FIELDS  = 5

Const MIN_NUMBER_OF_LINES = 10 	'<--- Must be set to a minimum number. if the number of lines in the file are less than this, 
								'    a backup is created and the "Good File" will be an empty file that should be refused for import in the import sproc
								'	  The Bad File will contain a line describing the failure
...
'			* Each array has a commented description at the end of the line. i.e. '<-- comments... The apostrophe MUST be in front of the description, or the script will error.
...
MaxLengthArray             = Array(999, 999, 999, 999, 3) '<-- Numeric - to NOT test the max length, use a high number like 999
MinLengthArray             = Array(0, 0, 0,0,3) '<-- Numeric - to NOT test the min length, use 0 (the number zero)
NumericOnlyArray           = Array(True, False, False, False, True) '<-- Boolean - True = Must contain only numeric characters, False = Don't test field
MustContainValueArray     	= Array("", "", "","HR|ADMIN|SALES|SUPPORT|FINANCE","") '<-- Text - provide empty string for fields not being tested, Pipe char as delimiter (i.e. "Employee|Department|Admin")
MustNotContainValueArray 	= Array("", "", "","","600") '<-- Text - provide empty string for fields not being tested, Pipe char as delimiter

'<----- END Test Config Arrays ------->
...
Function MustNotContainValue(FieldArray, LineString)
	...
	For i = 0 To UBound(FieldArray)
		...
		If MustNotContainValueArray(i) <> "" Then
			...
			'loop through the values list to see if the field equals one of them, if so, the test fails, no point in going further
			For s = 0 To UBound(ValsArray)
				If UCase(StripNonData(FieldArray(i))) = UCase(ValsArray(s)) Then
					NotInArray = False
					Exit For

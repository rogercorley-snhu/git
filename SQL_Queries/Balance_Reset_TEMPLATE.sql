...
Description:	Set of SQL queries created to completed the following tasks:
				1.)	Allow the technician to set a payment post transaction date @_TDATE.
				2.)	Use @_TDATE to determine Cycle BeginDate and EndDate.
				3.)	Assign BeginDate = @_BEGINDATE and EndDate = @_ENDDATE.
				4.)	Use dates and following filters to caclulate all transactions 
					for an account.
...
/*							MODIFY DATE VALUE ONLY!!
							DO NOT MODIFY TIME VALUE!!
...
FROM 		(SELECT a.AccountNo, b.BadgeNo 
				FROM tblAccountOHD AS a 
					join tblBadgesOHD AS b on a.AccountNo = b.AccountNo) AS d 
				,RESETS AS c
